...
function generate_challenge_table() {
  ...
  help_link = $("<a></a>")
    .attr("href", browserAPI.runtime.getURL("/html/help.html"))
    .attr("target", "_blank")
    .text("help");
  options_link = $("<a></a>")
    ...
    .text("options");
  website_link = $("<a></a>")
    .attr("href", "https://running-challenges.co.uk")
    .attr("target", "_blank")
    .text("website");
...
function add_table_break_row(table, title, help) {
  ...
  if (help !== undefined) {
    tooltip =
      '<span style="font-size: 10px; vertical-align: middle; cursor: default" title="' +
      help +
      '">[?]</span>';
  }
  main_row.append(
    $("<th/>")
      .attr("colspan", 6)
      .append("<span><b>" + title + "</b></span> " + tooltip),
  );
...
function get_tbody_content(challenge, userData) {
  var content = $("<tbody></tbody>").attr(
    "id",
    get_tbody_content_id(challenge),
  );
...
function get_flag_icon(country, height, width) {
  ...
  flag_img.attr(
    ...
    browserAPI.runtime.getURL(
      "/images/flags/png/" + country.flag_icon + ".png",
    ),
  );
...
function get_challenge_icon(challenge, height, width) {
  ...
  if (challenge.badge_icon !== undefined) {
    ...
    badge_img.attr(
      "src",
      browserAPI.runtime.getURL(
        "/images/badges/" + challenge.badge_icon + ".png",
      ),
    );
    badge_img.attr("alt", challenge.name);
    badge_img.attr("title", challenge.name);
...
function get_challenge_header_row(challenge, data) {
  ...
  if (has_geo_data(data) && challenge.has_map === true) {
    challenge_map_link = $("<span/>")
      .attr("id", challenge_map_link_id)
      .html('<span style="cursor: default">show map</span>')
      .click(function () {
  ...
  if (challenge.summary_text !== undefined) {
    ...
    if (
      challenge.subparts_completed_count !== undefined &&
      challenge.subparts_count !== undefined
    ) {
  ...
  if (challenge.complete) {
    main_row.append(
      $("<img/>")
        .attr("src", browserAPI.runtime.getURL("/images/badges/tick.png"))
        .attr("width", 24)
        .attr("height", 24),
    );
  }
...
function generateExplorerTableEntry(table, data) {
  ...
  if (data.geo_data === undefined) {
    ...
    var infoRow = $("<tr/>").append(
      $(
        '<td colspan="4" align="center">No parkrun event location information, unable to generate explorer results</td>',
      ),
    );
...
function createVoronoiMapPrototype() {
  ...
  L.VoronoiLayer = L.Layer.extend({
    ...
    _update: function () {
      ...
      this_container.setAttribute(
        "style",
        "margin-left: " + top_left.x + "px; margin-top: " + top_left.y + "px",
      );
      ...
      // them in.
      voronoi_extent_left = [
        Math.max(top_left.x, map_point_left_edge.x),
        Math.max(top_left.y, map_point_left_edge.y),
      ];
      voronoi_extent_right = [
        Math.min(top_left.x + size.x, map_point_right_edge.x),
        Math.min(top_left.y + size.y, map_point_right_edge.y),
      ];
      ...
      var cell_group = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "g",
      );
      cell_group.setAttribute(
        "transform",
      ...
      $.each(voronoi_polygons, function (index, cell) {
        ...
        var item_circle = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "circle",
        );
        ...
        item_circle.setAttribute(
          "stroke",
          filtered_points[index].circleColourLine,
        );
        ...
        if (zoomScaleOptions.eventNameVisible) {
          item_text = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "text",
          );
          item_text.setAttribute("x", cell.data.x);
          item_text.setAttribute(
            "y",
            cell.data.y + zoomScaleOptions.eventPointRadius + 8,
          ); // Move the text down below the point, plus some padding
          item_text.setAttribute("text-anchor", "middle");
          item_text.setAttribute(
            "font-size",
            zoomScaleOptions.eventNameTextSize + "px",
          );
        ...
        var item_path = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "path",
        );
        item_path.setAttribute(
          "d",
          "M " + get_voronoi_poly(cell).join(" L ") + " Z",
        );
...
function drawExplorerMap(divId, data) {
  ...
  var tilelayer_openstreetmap = L.tileLayer(
  ...
  console.log(
    "Creating voronoi map with cell renderer = " + explorerCellRenderer,
  );
  ...
  $.each(data.geo_data.data.countries, function (countryName, countryInfo) {
    ...
    var countryChildEventsCount =
      countryCompletionInfo[countryName].childEventsCount;
    var countryChildEventsCompletedCount =
      countryCompletionInfo[countryName].childEventsCompletedCount;
    ...
    if (countryChildEventsCount > 0) {
      if (event_has_valid_location(countryInfo)) {
        ...
        // completion numbers
        var marker_tooltip_text =
          countryName +
          " " +
          countryChildEventsCompletedCount +
          "/" +
          countryChildEventsCount;
          ...
          var pie_marker = L.piechartMarker(lat_lon, {
            ...
            data: [
              ...
              },
              {
                name: "Not Run",
                value:
                  countryChildEventsCount - countryChildEventsCompletedCount,
                style: {
  ...
  $.each(data.geo_data.data.events, function (event_name, event_info) {
    if (event_info.country_id == countryId) {
      if (event_has_valid_location(event_info)) {
        ...
        marker.bindPopup(
          get_parkrun_popup(event_name, event_info, {
            distance: false,
            completed_info: map_data.events_completed_map,
          }),
        );
...
function create_challenge_map_voronoi(_divId, challenge_data, data) {
  ...
  console.log(
    "Initialising the challenge voronoi map container for id=" + map_element_id,
  );
  ...
  var cellRenderer = function (vmap, data) {
    ...
    var reference_parkrun_name = calculate_average_parkrun_event_name(
      parkrun_results,
      geo_data,
    );
    ...
    eventsByDistance = computeDistanceToParkrunsFromEvent(
      geo_data,
      reference_parkrun,
    );
...
function create_challenge_map_standard(map_id, challenge_data, data) {
  ...
  $.each(
    challenge_data.completed_qualifying_events,
    function (event_name, event_info) {
      if (!events_on_the_map.includes(event_name)) {
        if (event_has_valid_location(event_info)) {
          // Make sure they are numbers
          var lat_lon = [+event_info.lat, +event_info.lon];
          var popup = get_parkrun_popup(event_name, event_info, {
            distance: true,
          });
          var marker = L.marker(lat_lon, {
            icon: events_complete_icon,
          }).bindPopup(popup);
          if (
            data.info.is_our_page &&
            data.info.has_home_parkrun &&
            data.user_data.home_parkrun_info.name == event_name
          ) {
            popup = get_parkrun_popup(event_name, event_info, {
              distance: false,
            });
            marker = L.marker(lat_lon, {
              icon: events_complete_icon_home,
            }).bindPopup(popup);
  ...
  if (data.info.is_our_page) {
    $.each(
      challenge_data.nearest_qualifying_events,
      function (event_name, event_info) {
        if (!events_on_the_map.includes(event_name)) {
          if (event_has_valid_location(event_info)) {
            // Make sure they are numbers
            ...
            var marker = L.marker(lat_lon, {
              icon: events_nearest_incomplete_icon,
            }).bindPopup(popup);
            if (
              data.info.is_our_page &&
              data.info.has_home_parkrun &&
              ...
              marker = L.marker(lat_lon, {
                icon: events_nearest_incomplete_icon_home,
              }).bindPopup(popup);
              added_home_parkrun = true;
            }
  ...
  $.each(
    challenge_data.all_qualifying_events,
    function (event_name, event_info) {
      ...
      if (!events_on_the_map.includes(event_name)) {
        if (event_has_valid_location(event_info)) {
          ...
          if (
            ...
            marker = L.marker(lat_lon, {
              icon: events_incomplete_icon_home,
            }).bindPopup(popup);
            added_home_parkrun = true;
          }
  ...
  if (
    data.info.is_our_page &&
    data.info.has_home_parkrun &&
    added_home_parkrun == false
  ) {
    if (event_has_valid_location(data.user_data.home_parkrun_info)) {
      var lat_lon = [
        +data.user_data.home_parkrun_info.lat,
        +data.user_data.home_parkrun_info.lon,
      ];
  ...
  if (challenge_data.complete == false) {
    if (
      data.info.is_our_page &&
      data.info.has_home_parkrun &&
      events_nearest_incomplete.getLayers().length > 0
    ) {
...
function get_parkrun_popup(event_name, event_info, custom_options) {
  ...
  if (event_info.event_url) {
    ...
    event_name_link =
      '<div style="text-align:center"><a href="' +
      event_info.local_url +
      "/" +
      event_info.shortname +
      '" target="_blank">' +
      event_name +
      " parkrun</a></div>";
  }
  ...
  if (event_info.distance !== undefined && options.distance) {
    popup =
      ...
      event_info.distance.toFixed(1) +
      " km away</div>";
  }

  if (
    custom_options.completed_info &&
    event_name in custom_options.completed_info
  ) {
    popup =
      popup +
      '<br/><div style="text-align:center">First attended: ' +
      custom_options.completed_info[event_name][0].date +
      "</div>";
  }
  return popup;
}
...
function drawExplorerDataTable(table, data) {
  ...
  const worldCompletionFractionString =
    worldEventsCompletedCount + "/" + worldEventsCount;
  ...
  const alphabeticallySortedCountries = Object.keys(
    countryCompletionInfo,
  ).sort();
  ...
  $.each(alphabeticallySortedCountries, function (idx, countryName) {
    ...
    if (countryInfo.childEventsCount > 0) {
      ...
      var countryCompletionPercentage =
        countryInfo.childEventsCompletedCount / countryInfo.childEventsCount;
      var countryCompletionFractionString =
        countryInfo.childEventsCompletedCount +
        "/" +
        countryInfo.childEventsCount;
      ...
      row.append(
        $("<td/>")
          .append(
            get_explorer_flag(countryName, countryCompletionPercentage > 0),
          )
          .append($("<a/>").attr("name", countryName)),
      );
...
function generate_standard_table_entry(challenge, table, data) {
  ...
  challenge.subparts_detail.forEach(function (subpart_detail) {
    ...
    if (subpart_detail["badge"] !== undefined) {
      console.log(
        "Adding a badge to the table - " +
          JSON.stringify(subpart_detail["badge"]),
      );
      subpart_row.append(
        $("<td></td>").append(
          get_challenge_icon(subpart_detail["badge"], 24, 24),
        ),
      );
    } else {
      ...
      if (stat_info.url !== undefined) {
        row.append(
          $("<td/>").append(
            $("<a/>", {
              href: stat_info.url,
              text: stat_info.value,
              target: "_blank",
              title: stat_info.help,
...
function isChallengeHidden(challengeShortname, userData) {
  ...
  if (userData !== undefined) {
    if (
      userData["challengeMetadata"] !== undefined &&
      challengeShortname in userData["challengeMetadata"]
    ) {
      if ("hidden" in userData.challengeMetadata[challengeShortname]) {
        isHidden = userData.challengeMetadata[challengeShortname].hidden;
        console.log(
          "isChallengeHidden found existing saved state: " + isHidden,
        );
      }
    }
  }
