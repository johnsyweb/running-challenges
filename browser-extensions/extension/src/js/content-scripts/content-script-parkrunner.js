    ...
    $("h3#volunteer-summary", parent).each(function (index) {
      ...
      $("tbody>tr", results_table).each(function (role_index) {
        ...
        normalised_volunteer_role =
          get_normalised_volunteer_role(volunteer_role);

        if (!(normalised_volunteer_role in completed_volunteer_roles)) {
          completed_volunteer_roles[normalised_volunteer_role] = 0;
        }
        completed_volunteer_roles[normalised_volunteer_role] += parseInt(
          volunteer_role_quantity,
        );
      });
...
function parsePageAthleteInfo() {
  ...
  var mainAthleteTag = $(
    "div[id=main]>div[id=primary]>div[id=content]>h2",
  ).first();
...
function parse_results_table() {
  ...
  get_table("results", get_localised_value("table_all_results")).each(
  ...
  if (parkruns_completed_sorted.length > 0) {
    console.log(
      "Sorted parkruns, first: " +
        parkruns_completed_sorted[0].date +
        " last: " +
        parkruns_completed_sorted[parkruns_completed_sorted.length - 1].date,
    );
  }
...
function create_skeleton_elements(id_map) {
  ...
  var running_challenges_messages_div = $("<div/>").attr(
    "id",
    id_map["messages"],
  );
  ...
  var running_challenges_main_table_div = $("<div/>").attr(
    "id",
    id_map["main"],
  );
  get_table("results", get_localised_value("table_all_results")).before(
    running_challenges_main_table_div,
  );
  ...
  if (result.badgesAwarded !== undefined) {
    if (result.badgesAwarded.length > 0) {
      ...
      $.each(result.badgesAwarded, function (index, badge) {
        badge_info.push({
          name: badge.name,
          icon: browserAPI.runtime.getURL(
            "/images/badges/" + badge.badge_icon + ".png",
          ),
    ...
    if (result.complete == true) {
      badge_info = {
        name: result.name,
        icon: browserAPI.runtime.getURL(
          "/images/badges/" + result.badge_icon + ".png",
        ),
        link: "#" + result.shortname,
      ...
      badge_info = {
        ...
        icon: browserAPI.runtime.getURL(
          "/images/badges/" + result.partial_completion_badge_icon + ".png",
        ),
        link: "#" + result.shortname,
...
function get_volunteer_badge(result) {
  ...
  if (result.complete == true) {
    ...
    if (result.subparts_completed_count >= 25) {
      badge_info.icon = browserAPI.runtime.getURL(
        "/images/badges/" + result.badge_icon + "-3-stars.png",
      );
      badge_info.name += " (25+ times)";
    } else if (result.subparts_completed_count >= 10) {
      badge_info.icon = browserAPI.runtime.getURL(
        "/images/badges/" + result.badge_icon + "-2-stars.png",
      );
      badge_info.name += " (10+ times)";
    } else if (result.subparts_completed_count >= 5) {
      badge_info.icon = browserAPI.runtime.getURL(
        "/images/badges/" + result.badge_icon + "-1-star.png",
      );
...
function create_page() {
  ...
  load_data()
    ...
    .then((volunteer_data) => {
      ...
      process_data_and_update_ui(
        id_map,
        parsed_results_data,
        parsed_volunteer_data,
        loaded_geo_data,
        loaded_user_data,
      );
    })
...
function load_volunteer_data() {
  return fetch_volunteer_data(get_athlete_id()).then((results) => {
    set_progress_message("Loaded volunteer data");
    return parse_volunteer_table(results);
  });
}

function process_data_and_update_ui(
  id_map,
  parsed_results_data,
  parsed_volunteer_data,
  loaded_geo_data,
  ...
  data.info.has_challenge_running_results =
    !!data.challenge_results.running_results;
  data.info.has_challenge_volunteer_results =
    !!data.challenge_results.volunteer_results;
  ...
  if (!has_geo_data(data)) {
    errors.push(
      "! Unable to fetch parkrun event location data: Stats, Challenges, and Maps requiring locations are not available !",
    );
  }
...
function handle_error(error) {
  ...
  set_progress_message(`Error: ${error}. Stack: ${error.stack}.`);
}
